# Тесты проекта KIKU

## Структура тестов

```
__tests__/
├── unit/                    # Unit тесты (быстрые, изолированные)
│   └── utils/               # Тесты утилит и вспомогательных функций
│
├── integration/             # Интеграционные тесты (средняя сложность)
│   ├── app/                 # Тесты запуска приложения
│   │   └── startup.test.tsx
│   └── contexts/            # Тесты контекстов
│       ├── MonitoringContext.test.tsx
│       ├── ParentalControlsContext.test.tsx
│       └── UserContext.test.tsx
│
└── e2e/                     # E2E тесты (полные процессы)
    ├── screens/             # Тесты экранов
    │   └── monitoring.test.tsx
    └── processes/           # Тесты процессов жизнедеятельности
        ├── ai-analysis.test.tsx
        ├── sos-process.test.tsx
        └── lifecycle.test.tsx
```

## Быстрый старт

### Запуск всех тестов
```bash
bun test
```

### Запуск по категориям
```bash
# Только unit тесты (быстро)
bun run test:unit

# Только integration тесты
bun run test:integration

# Только E2E тесты (медленно)
bun run test:e2e
```

### Запуск с покрытием
```bash
bun run test:coverage
```

### Watch mode (для разработки)
```bash
bun test --watch
```

## Что тестируется

### ✅ Unit тесты (`unit/`)
- Утилиты и вспомогательные функции
- Чистые функции без побочных эффектов
- Быстрые тесты (< 10ms каждый)

### ✅ Integration тесты (`integration/`)

#### Запуск приложения (`app/startup.test.tsx`)
- Инициализация всех провайдеров
- Загрузка данных из AsyncStorage
- Обработка ошибок при старте

#### Контексты (`contexts/`)
- **MonitoringContext** - мониторинг чатов, анализ сообщений, алерты
- **ParentalControlsContext** - родительские настройки, SOS, контакты
- **UserContext** - пользовательские данные, профили, дети

### ✅ E2E тесты (`e2e/`)

#### Экраны (`screens/`)
- Полный flow работы с экранами
- Навигация между экранами
- Взаимодействие пользователя с UI

#### Процессы (`processes/`)
- **ai-analysis.test.tsx** - полный цикл AI анализа
- **sos-process.test.tsx** - полный цикл SOS
- **lifecycle.test.tsx** - полный цикл жизнедеятельности приложения

## Примеры использования

### Тестирование нового контекста

```typescript
// __tests__/integration/contexts/YourContext.test.tsx
import { renderHook, waitFor, act } from '@testing-library/react-native';
import { YourProvider, useYourContext } from '@/constants/YourContext';

const createWrapper = () => {
  return ({ children }: { children: React.ReactNode }) => (
    <YourProvider>{children}</YourProvider>
  );
};

describe('YourContext Integration', () => {
  it('должен инициализироваться правильно', async () => {
    const { result } = renderHook(() => useYourContext(), {
      wrapper: createWrapper(),
    });

    await waitFor(() => {
      expect(result.current.isLoading).toBe(false);
    });
  });
});
```

### Тестирование нового процесса

```typescript
// __tests__/e2e/processes/your-process.test.tsx
describe('Your Process E2E', () => {
  it('должен пройти полный цикл процесса', async () => {
    // 1. Начальное состояние
    // 2. Действие пользователя
    // 3. Проверка результата
    // 4. Проверка побочных эффектов
  });
});
```

## Best Practices

1. **Изоляция** - каждый тест независим
2. **Моки** - все внешние зависимости замокированы
3. **Arrange-Act-Assert** - четкая структура теста
4. **Ожидание** - используйте `waitFor` для async операций
5. **Очистка** - `beforeEach` для сброса состояния

## CI/CD

Все тесты автоматически запускаются в CI/CD:
```bash
bun run ci:all  # Установка → Линт → TypeScript → Тесты
```

## Покрытие

Цель покрытия: **> 80%** для критических компонентов

Проверить покрытие:
```bash
bun run test:coverage
```

Отчет будет в `coverage/` директории.

## Troubleshooting

### Тесты падают с таймаутом
- Увеличьте `testTimeout` в `jest.config.js`
- Проверьте моки async операций

### Тесты падают из-за моков
- Проверьте что все зависимости замокированы в `jest.setup.js`
- Убедитесь что моки правильно настроены

### Тесты не находят модули
- Проверьте `moduleNameMapper` в `jest.config.js`
- Убедитесь что пути правильные (`@/` алиас)

## Дополнительная информация

- 📖 [TESTING_GUIDE.md](../TESTING_GUIDE.md) - Подробное руководство
- 📖 [QUICKSTART.md](../QUICKSTART.md) - Быстрый старт проекта
- 📖 [PROJECT_SUMMARY.md](../PROJECT_SUMMARY.md) - Описание проекта
