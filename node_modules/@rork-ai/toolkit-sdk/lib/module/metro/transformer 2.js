...
/**
 * Detects the Expo SDK version from package.json content
 */
function detectExpoVersion() {
  try {
    ...
    if (fs.existsSync(packageJsonPath)) {
      ...
      const version = dependencies.expo;
      if (version) {
        // Remove ^ or ~ prefix
        const cleanVersion = version.replace(/^[\^~]/, "");
        if (cleanVersion.startsWith("54.")) return "54";
        if (cleanVersion.startsWith("53.")) return "53";
      }
    }
  } catch {
...
function isRootLayout(filename) {
  return (filename.includes("app/_layout.tsx") || filename.includes("app/_layout.js")) && !filename.includes("app/(") // Exclude grouped layouts
  ;
}
...
function wrapLayoutWithProviders(src, environment) {
  ...
  if (environment === "development") {
    ...
    const devImport = version === "54" ? "import { RorkDevWrapper } from '@rork-ai/toolkit-sdk/v54';" : "import { RorkDevWrapper } from '@rork-ai/toolkit-sdk/v53';";
    wrappers.push({
      import: devImport,
      name: "RorkDevWrapper"
    });
  ...
  wrappers.push({
    import: "import { AnalyticsProvider } from '@rork-ai/toolkit-sdk';",
...
async function transform(props) {
  const environment = props.options?.dev ?? process.env.NODE_ENV === "development" ? "development" : "production";
