-- SQL схема с безопасным удалением существующих политик перед созданием
-- Используйте этот файл, если нужно пересоздать политики
...
DROP POLICY IF EXISTS "Users can view their own sync status" ON sync_status;
...
-- Теперь создаем таблицы (из SQL_РОДИТЕЛЬ_РЕБЕНОК_ДЛЯ_SUPABASE.sql)
